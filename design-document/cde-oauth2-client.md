...
该包用于存放项目中设计的实体，本项目中设计4个entity，应该有 `BaseRequestInfo.java`, `AuthorizationCodeRequestInfo.java`, `AccessTokenRequestInfo.java`, `UserRequestInfo.java`.

1、BaseRequestInfo.java 是个基础类，声明共有的属性，用于继承。
...
3、AuthorizationCodeRequestInfo.java 用于和github交互时登录授权获取code的请求传递参数的entity.
...
| state 		| long           | 可以取任意值，github认证服务器会原封不动地返回这个值 | 可读取
...
| code 	| String         | 用户登录授权成功后，通过github回调而获取的code,可用于请求token | 可读取
...
getAuthenticationUrl(): 该方法对应接口 `/authentication-url`，请求方式为get 。调用该接口，返回值是github授权登录请求code的完整 `url`
...
callback(): 该方法对应接口 `/callback` ，用于github验证服务端的回调。回调返回参数 `code` 和 `state` , code 可用于请求token，state可用于验证授权登录的唯一性。

|  参数          | 返回值类型  | 描述
|---------------------------|--------|---------
|参数1，code为github回调中所获取；参数2，state为github回调所获取    |   List    |步骤1，声明变量List list，作为返回值；步骤2，调用service包下AccessTokenRequestService的方法getAccessTokenByCode()，获取得到token；步骤3，根据步骤2中获取得到的token值，调用 service 包下 UserInfoRequestService 的方法getUserWithAccessToken()，获取得到用户的基本信息；步骤4，根据步骤三中所得的数据对步骤1声明的变量list赋值，并且将其返回
...
|  参数          | 返回值类型  | 描述
|---------------------------|--------|---------
|    |   String    |步骤1，声明变量String url；步骤2，调用builder包下RequestInfoBuilder的方法buildAuthorizationCodeRequestInfo()，获取AuthorizationCodeRequestInfo实例；步骤3，通过UriComponentsBuilder组装url；步骤4，调用UriComponentsBuilder的toUriString()方法生成url，并将其赋值予步骤1中声明的变量url，将其返回|
...
|  参数          | 返回值类型  | 描述
|---------------------------|--------|---------
| 参数1， code 参数为github回调获取得到的code值   |   AccessTokenRequestInfo    |步骤1，根据domain包下AccessTokenRequestInfo的构造方法获取到该entity的实例；步骤2，将该实例返回|
...
|  参数          | 返回值类型  | 描述
|---------------------------|--------|---------
| 无   |   AuthorizationCodeRequestInfo    |步骤1，声明一个局部变量state，取值为随机生成的数；步骤2，根据domian包下UserRequestInfo的构造方法获取到该entity的实例；步骤3，将该实例返回|

#### agent

该包下的类是封装了调用第三方API(github)的实现，新建java类 `RequestAgent.java` 
